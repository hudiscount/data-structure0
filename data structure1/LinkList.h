...
bool Union(LinkList<ElemType>& A, LinkList<ElemType>& B)
{
	...
	else
	{
		...
		Node<ElemType>* flaga=new Node<ElemType>;        //此处曾有警告！
...
// 操作结果：根据数组v中的元素构造单链表
{
	...
	for (int i = 0; i < n; i++) {
		p = new Node<ElemType>(v[i], NULL);     //p->next = new Node<ElemType>(v[i], NULL);
...
{
	Node<ElemType>* p = head->next;    //Node<ElemType>* p = head->next;
	while (p != NULL) {
		head->next = p->next;
		delete p;
		p = head->next;
...
{
	Node<ElemType>* p = head; //Node<ElemType>* p = head->next;
	while (p != NULL) {
