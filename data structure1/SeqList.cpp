...
template<class ElemType>
inline Status SeqList<ElemType>::DelSameItem(ElemType& e)
{
	int flag = 0;
	for (int i = 0; i < length; i++)
	{
	...
	if (flag == 0)
		...
		return SUCCESS;     //返回成功删除元素
}

template<class ElemType>
inline Status SeqList<ElemType>::Reverse()
{
	int i, j;
...
template<class ElemType>
inline void SeqList<ElemType>::ShowSeqList()
{
	for (int i = 0; i < length; i++)
	{
		cout << elems[i];
